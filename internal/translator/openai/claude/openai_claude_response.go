		...
		if toolCalls := delta.Get("tool_calls"); toolCalls.Exists() && toolCalls.IsArray() {
			...
			toolCalls.ForEach(func(_, toolCall gjson.Result) bool {
				...
				if function := toolCall.Get("function"); function.Exists() {
					...
					// Buffer the arguments here. The actual incremental
					// emission happens AFTER content_block_start has been
					// emitted (see "incremental flush" block below) — until
					// then we have no block index to attach the delta to.
				...
				// Incremental flush: once content_block_start has been
				// emitted, stream any newly-buffered args as input_json_delta
				// chunks instead of waiting for finish_reason. This keeps
				// long Write/Edit calls under the Cloudflare 100s
				// proxy-read window. EmittedArgsLen tracks how many bytes
				// have already been forwarded so the closing flush only
				// sends the remainder.
				if accumulator.StartEmitted && !param.ContentBlocksStopped {
					blockIndex := param.toolContentBlockIndex(index)
					accumulated := accumulator.Arguments.String()
					if len(accumulated) > accumulator.EmittedArgsLen {
						pending := accumulated[accumulator.EmittedArgsLen:]
						...
						inputDeltaJSON, _ = sjson.SetBytes(inputDeltaJSON, "delta.partial_json", pending)
